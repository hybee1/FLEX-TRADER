package com.synogiestechnologies.flex_trader_reactive_websocket_server.JwtService;


import com.synogiestechnologies.flex_trader_reactive_websocket_server.CallToAuthBackend.CallToAuthBackend;
import com.synogiestechnologies.flex_trader_reactive_websocket_server.DTORequest.TokenRevokedRequest;
import com.synogiestechnologies.flex_trader_reactive_websocket_server.DTOResponse.TokenRevokedResponse;
import com.synogiestechnologies.flex_trader_reactive_websocket_server.Exceptions.ExpiredTokenException;
import com.synogiestechnologies.flex_trader_reactive_websocket_server.Exceptions.InvalidTokenException;
import com.synogiestechnologies.flex_trader_reactive_websocket_server.Exceptions.SigningKeyException;
import com.synogiestechnologies.flex_trader_reactive_websocket_server.SubscriptionPlanType.SubscriptionPlanType;
...
import static com.synogiestechnologies.flex_trader_reactive_websocket_server.SubscriptionPlanType.SubscriptionPlanType.fromDisplayName;
...
public class JwtService {
    ...
    public JwtService(CallToAuthBackend callToAuthBackend, WebClient authWebClient) {
    ...
    private Claims extractAllClaims(String token) {
        System.out.println("inside extractAllClaims IN REACTIVE WEBSOCKET-JWT-SERVICE");
        ...
        try{
            ...
            // Token is expired
            System.out.println("Token is expired IN REACTIVE WEBSOCKET-JWT-SERVICE. " +
                    "\nThe error message: " + e.getMessage() +
                    "\nThe error cause: " + e.getCause() );

            log.error("Token is expired IN REACTIVE WEBSOCKET-JWT-SERVICE. " +
                    "\nThe error message: " + e.getMessage() +
                    "\nThe error cause: " + e.getCause()
            );
            ...
            System.out.println("Token is invalid IN REACTIVE WEBSOCKET-JWT-SERVICE. " +
                    "\nThe error message: " + e.getMessage() +
                    "\nThe error cause: " + e.getCause() );

            log.error("Token is invalid IN REACTIVE WEBSOCKET-JWT-SERVICE. " +
                    "\nThe error message: " + e.getMessage() +
                    "\nThe error cause: " + e.getCause()
            );
...
///  use webclient to make to auth backend to check token revocation status
    public Mono<Boolean> isTokenRevokedReactive(String token) {
        return authWebClient.post()
                .uri("/api/authentication/revoked-token")
                .bodyValue(new TokenRevokedRequest(token))
                .retrieve()
                .onStatus(HttpStatusCode::isError, resp ->
                        Mono.error(new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE,
                                "Revocation service unavailable"))
                ) // in case auth-service is down so handle error gracefully
                .bodyToMono(TokenRevokedResponse.class)
                .map(res ->  res.getData().isRevoked() );
    }

    public Mono<Boolean> isTokenValidReactive(String token) {
        System.out.println("INSIDE isTokenValid");

        return Mono.just(isTokenExpired(token)) // make sure this also returns Mono<Boolean>
                .zipWith(isTokenRevokedReactive(token))
                .map(tuple -> {
                    boolean tokenHasExpired = tuple.getT1();
                    boolean tokenWasRevoked = tuple.getT2();
...
//            String subPlan = (String) websocketDetails.get("subPlan");
...
//
//            boolean result = roles.contains("ROLE_USER") && expiryDate >= Instant.now().toEpochMilli();
//            System.out.println("roles.contains(\"ROLE_USER\") && expiryDate >= Instant.now().toEpochMilli() = "
//                    + result);
//
//            return result;
//        }
...
//    }
    ...
    public Mono<Boolean> canConnectToWebsocketReactive(String token) {
        return Mono.fromCallable(() -> {
                    ...
                    System.out.println("INSIDE canConnectToWebsocket 3");
                    if (websocketDetails == null) {
                        System.out.println("INSIDE canConnectToWebsocket 3a");
                        System.out.println("websocketDetails is null — ");
                        log.warn("websocketDetails is null — denying connection");
                        return false;
                    }
...
//                            expiryDate >= Instant.now().toEpochMilli();
                ...
                .onErrorResume(e -> {
                    log.error("Error checking websocket connection: {}", e.getMessage());
                    return Mono.just(false);
                });
    }
